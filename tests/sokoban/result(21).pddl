(define (problem Sokoban)
		(:domain sokoban)
		(:objects C0 D0 E0 F0 A1 B1 C1 D1 E1 F1 A2 B2 C2 D2 E2 F2 G2 A3 B3 C3 D3 E3 F3 G3 H3 A4 B4 C4 D4 E4 F4 G4 H4 A5 B5 C5 D5 E5 F5 G5 H5 B6 C6 D6 E6 F6 G6 H6 B7 C7 D7 E7 F7 G7 H7 B8 C8 D8 E8 - position agent - guard)
		(:INIT  (isWall C0) (isWall D0) (isWall E0) (isWall F0) (isWall A1) (isWall B1) (isWall C1) (isWall F1) (isWall A2) (isWall F2) (isWall G2) (isWall A3) (isWall C3) (isWall G3) (isWall H3) (isWall A4) (isWall E4) (isWall H4) (isWall A5) (isWall B5) (isWall H5) (isWall B6) (isWall H6) (isWall B7) (isWall E7) (isWall F7) (isWall G7) (isWall H7) (isWall B8) (isWall C8) (isWall D8) (isWall E8)
 (at agent F3)
 (isBox C4) (isBox F4) (isBox D5) (isBox E5) (isBox D6)
 (haut D1 D2) (bas D2 D1)
 (gauche D1 E1) (droite E1 D1)
 (haut E1 E2) (bas E2 E1)
 (haut B2 B3) (bas B3 B2)
 (gauche B2 C2) (droite C2 B2)
 (gauche C2 D2) (droite D2 C2)
 (haut D2 D3) (bas D3 D2)
 (gauche D2 E2) (droite E2 D2)
 (haut E2 E3) (bas E3 E2)
 (haut B3 B4) (bas B4 B3)
 (haut D3 D4) (bas D4 D3)
 (gauche D3 E3) (droite E3 D3)
 (gauche E3 F3) (droite F3 E3)
 (haut F3 F4) (bas F4 F3)
 (gauche B4 C4) (droite C4 B4)
 (haut C4 C5) (bas C5 C4)
 (gauche C4 D4) (droite D4 C4)
 (haut D4 D5) (bas D5 D4)
 (haut F4 F5) (bas F5 F4)
 (gauche F4 G4) (droite G4 F4)
 (haut G4 G5) (bas G5 G4)
 (haut C5 C6) (bas C6 C5)
 (gauche C5 D5) (droite D5 C5)
 (haut D5 D6) (bas D6 D5)
 (gauche D5 E5) (droite E5 D5)
 (haut E5 E6) (bas E6 E5)
 (gauche E5 F5) (droite F5 E5)
 (haut F5 F6) (bas F6 F5)
 (gauche F5 G5) (droite G5 F5)
 (haut G5 G6) (bas G6 G5)
 (haut C6 C7) (bas C7 C6)
 (gauche C6 D6) (droite D6 C6)
 (haut D6 D7) (bas D7 D6)
 (gauche D6 E6) (droite E6 D6)
 (gauche E6 F6) (droite F6 E6)
 (gauche F6 G6) (droite G6 F6)
 (gauche C7 D7) (droite D7 C7)

)
		(:goal (AND (isBox D2) (isBox D4) (isBox C5) (isBox D5) (isBox E5)))
)
